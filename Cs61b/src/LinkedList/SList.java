...
public class SList {
  ...
  public void insertEnd(Object obj) {
    if (head == null) {
      ...
      node.changeNext(new SListNode(obj));
    }
    size++;
  }
  ...
  public void squish() {
    // Fill in your solution here.  (Ours is eleven lines long.)
	  ...
	  while(node != null && node.getNext() != null) {
		  if(node.getItem().equals(node.getNext().getItem())) {
			  node.deleteNext();
			  this.size--;
		  } else {
  ...
  public void twin() {
    // Fill in your solution here.  (Ours is seven lines long.)
	  SListNode node = this.head;
	  while(node != null) {
		  SListNode duNode = new SListNode(node.getItem(), node.getNext());
		  node.changeNext(duNode);
		  node = duNode.getNext();
		  size++; 
	  }
  }
